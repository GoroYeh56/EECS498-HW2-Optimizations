    ...
    while 1:
        ...
        while 1:
            ...
            for j in range(0,numplanes):
                fprime_for_plane_j = ((1 / (-a[:, j].T * x + b[j]).item(0)) * a[:, j]) # need to transpose????????
                fprimeprime_for_plane_j = (1 / (-a[:, j].T * x + b[j]).item(0)) * (1 / (-a[:, j].T * x + b[j]).item(0)) * a[:, j].T * a[:, j] ###YOUR CODE HERE###
                # print(a[:, j])

                fprime = fprime - fprime_for_plane_j; # put in the contribution of hyperplane j to fprimene_j)
            ...
            s = 0
            for i in range(numplanes):
                s += (a[:,i]*a[:,i].T) / ((-a[:,i].T*x + b[i])*(-a[:,i].T*x + b[i]))  
            fprimeprime = s # KEY !
            ...
            step =  -fprimeprime.I * fprime # 2*1 matrix
